    ...
    >
      <div className="container flex items-center justify-between px-4 mx-auto sm:px-6 lg:px-8 max-w-[1400px]">
        ...
        >
          ...
          {/* ডেস্কটপের জন্য আগের সাইজ ফিরিয়ে দেওয়া হয়েছে যাতে না ভাঙে */}
          <h1 className="text-xs sm:text-sm lg:text-[13px] xl:text-base font-extrabold tracking-widest text-white uppercase whitespace-nowrap drop-shadow-md">
            {t("nav_logo")}
          </h1>
        </Link>
        ...
        {/* =========================================
            2. DESKTOP MENU (Hidden on < 1024px)
            ========================================= */}
        {/* গ্যাপগুলো আগের অবস্থায় ফিরিয়ে দেওয়া হয়েছে */}
        <div className="justify-center hidden lg:flex lg:px-2 xl:px-4">
          <ul className="flex items-center lg:gap-4 xl:gap-6">
            {navLinks.map((item) => {
              const isActive =
                pathname === (item === "home" ? "/" : `/${item}`);
              return (
        ...
        <div className="items-center hidden gap-3 shrink-0 lg:flex xl:gap-5">
          ...
          >
            ...
            {/* ফন্ট সাইজ আগের অবস্থায় ফিরিয়ে দেওয়া হয়েছে */}
            <span className="lg:text-[10px] xl:text-xs font-bold tracking-widest text-white uppercase whitespace-nowrap">
              {t("nav_phone_number")}
            </span>
          </a>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, height: 0 }}
            transition={{ duration: 0.4, ease: [0.04, 0.62, 0.23, 0.98] }}
            className="absolute left-0 flex flex-col w-full overflow-hidden border-t border-white/5 bg-[#111827]/98 backdrop-blur-3xl lg:hidden top-full"
          >
            <div className="flex-1 px-6 py-8 pb-32 overflow-y-auto sm:py-12">
              <ul className="flex flex-col items-center gap-7 sm:gap-10">
                {navLinks.map((item, index) => {
                  const isActive =
                    pathname === (item === "home" ? "/" : `/${item}`);
                  return (
                    ...
                    >
                      <Link
                        to={item === "home" ? "/" : `/${item}`}
                        onClick={() => setIsMobileMenuOpen(false)}
                        className={`block text-base sm:text-lg md:text-xl font-bold uppercase tracking-[0.2em] transition-colors py-2 ${
                          isActive
                            ? "text-[#dbb671]"
                            : "text-gray-300 hover:text-white"
                        }`}
              ...
              >
                ...
                >
                  <img src={icons.call} alt="call" className="w-5 h-5" />
                  <span className="text-sm font-bold tracking-widest uppercase sm:text-base">
                    {t("nav_phone_number")}
                  </span>
                </a>
